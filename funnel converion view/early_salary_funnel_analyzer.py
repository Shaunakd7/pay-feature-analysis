#!/usr/bin/env python3
"""
Early Salary Funnel Analyzer
Uses exact same code as original funnel_analyzer.py but with Early Salary config
"""
...
FUNNEL_CONFIG = {
    ...
    # "time_period_days": 9000,  # Remove this line to make date filter optional
    "steps": [
        {
            "rank": 1,
...
class StandardFunnelAnalyzer:
    ...
    def validate_config(self, config: Dict[str, Any]) -> bool:
        ...
        required_fields = [
            ...
            'timestamp_column', 'partition_column', 'steps'  # Removed 'time_period_days'
        ]
    ...
    def check_event_availability(self, config: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        query = f"""
        SELECT 
            {config['event_column']} as event,
            COUNT(*) as count
        FROM `{config['data_source']}`
        WHERE {config['event_column']} in ('{events_list}')
        AND {filter_conditions}
        {'' if config.get('time_period_days') is None else f"AND date({config['timestamp_column']}) >= date_sub(current_date(), interval {config['time_period_days']} day)"}
        GROUP BY {config['event_column']}
        ORDER BY count DESC
        """
...
with funnel_base as (
    ...
    {'' if config.get('time_period_days') is None else f"and date(e.{config['timestamp_column']}) >= date_sub(current_date(), interval {config['time_period_days']} day)"}
