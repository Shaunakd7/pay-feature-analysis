...
app as app_version,context.os.name as operating_system,properties.entry_point as entry_point,    case 
...
select 
    user_id,
    session_id,
app_version,operating_system,entry_point,    screen_name,
    screen_entry_time,
    funnel_name,
    step_stage
...
with user_journey_summary as (
    select
        user_id,
        session_id,
app_version,operating_system,entry_point,        min(step_stage) as first_step,
...
, app_version, operating_system, entry_point)
,step_level_metrics as (
    select
        step_stage,
        screen_name,
app_version,operating_system,entry_point,        -- # of starters (users who reached this step)
        ...
        -- P50 time to reach this step
        percentile_cont(timestamp_diff(f.screen_entry_time, ujs.first_screen_time, second), 0.5) over() as p50_time_to_reach_seconds,
        ...
        -- P50 time to convert from this step
        percentile_cont(case when f.is_conversion_session = 1 then f.time_to_convert end, 0.5) over() as p50_time_to_convert_seconds,
        -- Avg sessions to reach this step
        avg(ujs.session_count) as avg_sessions_to_reach,
        -- P50 sessions to reach this step
        percentile_cont(ujs.session_count, 0.5) over() as p50_sessions_to_reach,
        ...
        -- P50 sessions to convert from this step
        percentile_cont(case when f.is_conversion_session = 1 then ujs.session_count, 0.5) over() as p50_sessions_to_convert
...
app_version,operating_system,entry_point,        -- # of starters (users who started the funnel)
        ...
        -- P50 time to reach e2e
        percentile_cont(case when last_step = 6 then total_time_seconds end, 0.5) over() as p50_time_to_reach_seconds,
        -- Avg time to convert e2e
        avg(case when last_step = 6 then total_time_seconds end) as avg_time_to_convert_seconds,
        -- P50 time to convert e2e
        percentile_cont(case when last_step = 6 then total_time_seconds end, 0.5) over() as p50_time_to_convert_seconds,
        -- Avg sessions to reach e2e
        avg(case when last_step = 6 then session_count end) as avg_sessions_to_reach,
        -- P50 sessions to reach e2e
        percentile_cont(case when last_step = 6 then session_count, 0.5) over() as p50_sessions_to_reach,
        -- Avg sessions to convert e2e
        avg(case when last_step = 6 then session_count end) as avg_sessions_to_convert,
        -- P50 sessions to convert e2e
        percentile_cont(case when last_step = 6 then session_count, 0.5) over() as p50_sessions_to_convert
    from user_journey_summary
    group by app_version,operating_system,entry_point)
select 
    'stage' as analysis_level,
    step_stage,
    screen_name,
app_version,operating_system,entry_point,    starters_count,
    completes_count,
    drop_offs_count,
    screen_conversion_rate,
    drop_off_rate,
    avg_time_to_reach_seconds,
    p50_time_to_reach_seconds,
    avg_time_to_convert_seconds,
    p50_time_to_convert_seconds,
    avg_sessions_to_reach,
    p50_sessions_to_reach,
    avg_sessions_to_convert,
    p50_sessions_to_convert,
    current_timestamp() as table_created_at
